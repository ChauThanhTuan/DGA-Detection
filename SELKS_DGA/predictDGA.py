...
def _get_prediction(domain_name, prob=None):
    ...
    if prob >= 0.5:
        ...
        cmd = f'curl -XPOST -H \'Authorization: Bearer dlkOW4xt2XCIUDrinP1B6d5cgb0Vxipv\' -H \'Content-Type: application/json\' http://{config.SELKS_IP}:{config.THEHIVE_PORT}/api/alert -d \'\x7B  "title": "DGA alert",  "description": "DGA alert: TheHive predict DGA domain: {} is DGA with probability {domain_name}",  "type": "external",  "source": "instance-{str(datetime.now())}",  "sourceRef": "alert-ref"\x7D\''
